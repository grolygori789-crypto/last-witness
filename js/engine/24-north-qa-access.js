...
function testerAuthorized(){
 ...
 return memoryRole===TESTER_ROLE
}
function setTesterAuthorized(value){
 memoryRole=value?TESTER_ROLE:"";
 try{if(value)sessionStorage.setItem(ROLE_KEY,TESTER_ROLE);else sessionStorage.removeItem(ROLE_KEY)}catch(_){}
 syncAccessButtons()
}
...
function status(text="",kind=""){
 ...
 node.textContent=text;node.className="north-qa-status"+(kind?" "+kind:"")
}
function notify(text){
 try{if(typeof showBadge==="function")showBadge(text)}catch(_){}
}
function setBusy(value){
 running=value;
 $$("#northQaModal button[data-north-qa-action]").forEach(button=>button.disabled=value)
}
function requireTester(){
 if(testerAuthorized())return true;
 status("Tester access is no longer active.","error");
 syncAccessButtons();return false
}
function waitFor(predicate,timeout=5000){
 return new Promise(resolve=>{
  const started=performance.now();
  const check=()=>{
   let passed=false;try{passed=Boolean(predicate())}catch(_){}
   if(passed){resolve(true);return}
...
function phaseLabel(){
 const chapter=Number(gs()?.chapter)||1;
 return chapter===4?String(chapter4Phase()||"UNRESOLVED"):"N/A"
}
function syncAccessButtons(){
 const active=testerAuthorized();
 const menu=$("#northQaMenuButton"),title=$("#northQaTitleButton"),owner=$("#developerMenuButton");
 if(menu)menu.style.display=active?"block":"none";
 if(title)title.style.display=active?"inline-flex":"none";
 if(active&&owner)owner.style.display="none"
}
function showQa(){
...
function showAuthFlash(){
 ...
 flash.classList.add("show");setTimeout(()=>{flash.classList.remove("show");showQa()},900)
}
function authenticateTester(event){
 event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();
 try{if(typeof lockDeveloperAccess==="function")lockDeveloperAccess()}catch(_){}
 setTesterAuthorized(true);
 const input=$("#devAccessCode");if(input)input.value="";
 $("#devAccessStatus")&&( $("#devAccessStatus").textContent="" );
 $("#devAccessModal")?.classList.remove("open");
 showAuthFlash()
}
function handleAccessAttempt(event){
 const input=$("#devAccessCode");if(!input)return;
 const code=String(input.value||"").trim().toLowerCase();
 if(code===TESTER_CODE){authenticateTester(event);return}
 setTimeout(()=>{
  let ownerActive=false;
  try{ownerActive=typeof developerUnlocked==="function"&&developerUnlocked()}catch(_){}
  if(!ownerActive){
   const access=$("#devAccessModal"),owner=$("#developerMenuButton");
   ownerActive=Boolean(access&&!access.classList.contains("open")&&owner&&getComputedStyle(owner).display!=="none")
  }
  if(ownerActive)lockTesterAccess()
 },0)
}
function guardOwnerConsole(event){
 if(!testerAuthorized())return;
 if(event.target.closest?.("#developerMenuButton,#developerModal")){
  event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();
  $("#developerModal")?.classList.remove("open");showQa()
 }
}

async function enterChapter1(){
 const fn=typeof startNewGame==="function"?startNewGame:window.startNewGame;
 if(typeof fn!=="function")throw new Error("Chapter I entry API unavailable");
 stopCurrentMedia();closeOverlays();fn();
 return Number(gs()?.chapter)===1&&gs()?.checkpoint==="ch1_start"
}
async function enterChapter2(){
 const fn=typeof developerJump==="function"?developerJump:window.developerJump;
 if(typeof fn!=="function")throw new Error("Chapter II entry API unavailable");
 stopCurrentMedia();closeOverlays();fn("office2");
 return await waitFor(()=>Number(gs()?.chapter)===2&&activeScreen()==="office2",2500)
}
async function enterChapter3(){
 const integration=window.LastWitnessChapter2Integration;
 if(typeof integration?.ensureProductionRuntime!=="function")throw new Error("Chapter III loader unavailable");
 status("Loading Chapter III runtime...");
 await integration.ensureProductionRuntime();
 if(typeof window.LastWitnessChapter3?.startFromChapter2!=="function")throw new Error("Chapter III entry API unavailable");
 stopCurrentMedia();closeOverlays();document.title="Last Witness — The Borrowed Minutes";
 window.LastWitnessChapter3.startFromChapter2();
 return await waitFor(()=>Number(gs()?.chapter)===3&&activeScreen().startsWith("chapter3"),5000)
}
async function enterChapter4(phase){
 const nav=window.LastWitnessDeveloperPhaseNavigation;
 const id=CHAPTER4_IDS[phase];
 if(!id||typeof nav?.run!=="function")throw new Error("Chapter IV phase navigation unavailable");
 return await nav.run(id)
}
async function runNavigation(action){
 ...
 try{
  ...
  console.error("LAST WITNESS North QA navigation failed",error);
  status("Unable to open the requested test entry.","error");notify("Unable to open test entry");return false
 }finally{setBusy(false)}
}
async function restartCurrent(){
 if(!requireTester()||running)return;
 const chapter=Number(gs()?.chapter)||1;
 if(chapter===1){await runNavigation("chapter1");return}
 if(chapter===2){await runNavigation("chapter2");return}
 if(chapter===3){await runNavigation("chapter3");return}
 if(chapter===4){
  const phase=chapter4Phase();
  if(phase){await runNavigation("chapter4phase"+phase);return}
 }
 status("Current chapter or phase could not be identified.","error")
}
function testInfo(){
 const s=gs()||{};
 return[
...
async function copyTestInfo(){
 if(!requireTester()||running)return;
 try{const copied=await copyText(testInfo());if(!copied)throw new Error("Clipboard copy failed");status("Test information copied.","success");notify("Test information copied")}catch(error){console.error("LAST WITNESS North QA copy failed",error);status("Unable to copy test information.","error")}
}
...
function createStyle(){
 if($("#northQaStyle"))return;
 const style=document.createElement("style");style.id="northQaStyle";
 style.textContent=`
 ...
 #northQaFlash{z-index:430}.north-qa-lock{border-color:rgba(181,63,56,.55)!important;color:#e29a94!important}
...
function createUi(){
 ...
 if(!$("#northQaModal")){
  ...
  modal.innerHTML=`<div class="modal-card"><div class="north-qa-head"><strong>NORTH QA ACCESS</strong><div class="north-qa-build">BUILD ${VERSION}</div><p class="north-qa-note">Use an Incognito window for isolated testing. QA navigation changes only the current browser test session.</p></div><div class="north-qa-section">TEST NAVIGATION</div><div class="north-qa-grid"><button class="dev-button" type="button" data-north-qa-action="chapter1">CHAPTER I</button><button class="dev-button" type="button" data-north-qa-action="chapter2">CHAPTER II</button><button class="dev-button" type="button" data-north-qa-action="chapter3">CHAPTER III</button><button class="dev-button" type="button" data-north-qa-action="chapter4phase1">CHAPTER IV · PHASE I</button><button class="dev-button" type="button" data-north-qa-action="chapter4phase2">CHAPTER IV · PHASE II</button><button class="dev-button" type="button" data-north-qa-action="chapter4phase3">CHAPTER IV · PHASE III</button><button class="dev-button" type="button" data-north-qa-action="chapter4phase4">CHAPTER IV · PHASE IV</button><button class="dev-button" type="button" data-north-qa-action="chapter4phase5">CHAPTER IV · PHASE V</button></div><div class="north-qa-section">SESSION</div><div class="north-qa-session"><button class="ghost" type="button" data-north-qa-action="restart">RESTART CURRENT CHAPTER / PHASE</button><button class="ghost" type="button" data-north-qa-action="copy">COPY TEST INFO</button><button class="ghost" type="button" data-north-qa-action="title">RETURN TO TITLE</button><button class="ghost north-qa-lock" type="button" data-north-qa-action="lock">LOCK TESTER ACCESS</button><button class="ghost" type="button" data-north-qa-action="close">CLOSE</button></div><div id="northQaStatus" class="north-qa-status" aria-live="polite"></div></div>`;
  document.body.appendChild(modal)
 }
 if(!$("#northQaFlash")){
  const flash=document.createElement("div");flash.id="northQaFlash";flash.className="dev-access-flash";flash.innerHTML="<div><strong>AUTHENTICATION SUCCESSFUL</strong><br><small>NORTH QA ACCESS UNLOCKED</small></div>";document.body.appendChild(flash)
 }
 if(!$("#northQaMenuButton")){
  const button=document.createElement("button");button.id="northQaMenuButton";button.className="menu-button";button.type="button";button.textContent="North QA Access";
  const restart=$("#restart");restart?.parentNode?.insertBefore(button,restart)
 }
 if(!$("#northQaTitleButton")){
  const button=document.createElement("button");button.id="northQaTitleButton";button.className="ghost";button.type="button";button.textContent="NORTH QA ACCESS";$("#title .title-ui")?.appendChild(button)
 }
}
function bindUi(){
 $("#northQaMenuButton")?.addEventListener("click",showQa);
 $("#northQaTitleButton")?.addEventListener("click",showQa);
...
function install(){
 ...
 createUi();bindUi();syncAccessButtons();installed=true;return true
}

window.LastWitnessNorthQA={version:VERSION,installed:true,install};

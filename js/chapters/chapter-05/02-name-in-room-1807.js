/* LAST WITNESS - Chapter V / Phase II: NAME IN ROOM 1807 0.22.22-c5p2r18
 * Production module update under base Runtime 0.22.22.
...
let trackBPrimePromise=null,trackBPrimed=false,trackBPrimeActive=false,trackBPrimeTimer=0,trackBPrimeStartTime=0,trackBStartCount=0;
let activeScoreMode="a";
const TRACK_B_AUDIBLE_PRIME_MS=140;
...
function scoreMedia(mode="a"){return media(mode==="b"?"ch5P2MusicB":"ch5P2MusicA")}
...
function clearTrackBPrimeTimer(){if(trackBPrimeTimer){clearTimeout(trackBPrimeTimer);trackBPrimeTimer=0}}
function prepareTrackBElement(){const b=scoreMedia("b");if(!b)return false;try{b.preload="auto";b.loop=true;b.muted=false;if(b.readyState===0)b.load()}catch(_){}return true}
function stopTrackBPlayback(reset=false){const b=scoreMedia("b");clearTrackBPrimeTimer();trackBPrimePromise=null;trackBPrimed=false;trackBPrimeActive=false;trackBPrimeStartTime=0;if(!b)return false;cancelFade(b);try{b.pause();b.volume=0;if(reset)b.currentTime=0}catch(_){}return true}
function trackBPosition(){return finiteMediaTime(scoreMedia("b"))}
function trackBPlaying(){return isPlaying(scoreMedia("b"))}
function waitForTrackBAdvance(mediaEl,fromTime,timeout=1500){const began=performance.now();return new Promise(resolve=>{let frame=0,done=false;const finish=ok=>{if(done)return;done=true;if(frame)cancelAnimationFrame(frame);resolve(Boolean(ok))};const step=()=>{if(!mediaEl||mediaEl.paused||mediaEl.ended)return finish(false);const t=finiteMediaTime(mediaEl);if(t>fromTime+.045||t<fromTime-.2)return finish(true);if(performance.now()-began>=timeout)return finish(false);frame=requestAnimationFrame(step)};frame=requestAnimationFrame(step)})}
function primeTrackBFromGesture(forceReset=true){
 const b=scoreMedia("b"),target=scoreTarget();clearTrackBPrimeTimer();trackBPrimed=false;trackBPrimeActive=false;if(target<=0){trackBPrimed=true;return trackBPrimePromise=Promise.resolve(true)}if(!b)return trackBPrimePromise=Promise.resolve(false);prepareTrackBElement();
 try{if(forceReset){b.pause();b.currentTime=0}b.loop=true;b.muted=false;b.volume=target}catch(_){return trackBPrimePromise=Promise.resolve(false)}
 const fromTime=finiteMediaTime(b);let result;try{result=b.play();trackBStartCount++}catch(_){return trackBPrimePromise=Promise.resolve(false)}
 trackBPrimeActive=true;trackBPrimeStartTime=performance.now();trackBPrimePromise=Promise.resolve(result).then(()=>waitForTrackBAdvance(b,fromTime,1500)).then(ok=>{if(!ok||!isPlaying(b)){trackBPrimeActive=false;trackBPrimed=false;try{b.pause();b.volume=0}catch(_){};return false}return new Promise(resolve=>{const elapsed=Math.max(0,performance.now()-trackBPrimeStartTime),delay=Math.max(0,TRACK_B_AUDIBLE_PRIME_MS-elapsed);trackBPrimeTimer=setTimeout(()=>{trackBPrimeTimer=0;if(!isPlaying(b)){trackBPrimeActive=false;trackBPrimed=false;resolve(false);return}try{b.volume=0}catch(_){};trackBPrimeActive=false;trackBPrimed=true;resolve(true)},delay)})}).catch(()=>{trackBPrimeActive=false;trackBPrimed=false;try{b.pause();b.volume=0}catch(_){};return false});return trackBPrimePromise
}
function startTrackBDirectFromGesture(reset=false){
 const b=scoreMedia("b"),target=scoreTarget();if(target<=0){trackBPrimed=true;activeScoreMode="b";pauseElement(scoreMedia("a"));return Promise.resolve(true)}if(!b)return Promise.resolve(false);prepareTrackBElement();clearTrackBPrimeTimer();trackBPrimeActive=false;trackBPrimed=false;try{if(reset){b.pause();b.currentTime=0}b.loop=true;b.muted=false;b.volume=target}catch(_){return Promise.resolve(false)}const fromTime=finiteMediaTime(b);let r;try{r=b.play();trackBStartCount++}catch(_){return Promise.resolve(false)}return Promise.resolve(r).then(()=>waitForTrackBAdvance(b,fromTime,1500)).then(ok=>{if(!ok)return false;trackBPrimed=true;activeScoreMode="b";pauseElement(scoreMedia("a"));foregroundGesturePending=false;return true}).catch(()=>false)
}
function promoteTrackBForNarin(){const b=scoreMedia("b"),target=scoreTarget();if(target<=0){activeScoreMode="b";pauseElement(scoreMedia("a"));return true}if(!b||!trackBPrimed||!isPlaying(b))return false;clearTrackBPrimeTimer();trackBPrimeActive=false;try{b.muted=false;b.volume=target}catch(_){return false}activeScoreMode="b";pauseElement(scoreMedia("a"));foregroundGesturePending=false;return true}
function holdConnectSilence(){const a=scoreMedia("a"),room=media("ch5P2RoomTone");if(a){cancelFade(a);try{a.volume=0}catch(_){}}if(room){cancelFade(room);try{room.volume=0;room.pause()}catch(_){}}const b=scoreMedia("b");if(b&&!trackBPrimeActive&&trackBPrimed)try{b.volume=0}catch(_){};return true}
function ensureTrackA(fromGesture=false){const a=scoreMedia("a"),b=scoreMedia("b"),target=scoreTarget();if(!a||target<=0)return false;if(isPlaying(b))stopTrackBPlayback(true);try{a.loop=true;a.muted=false;a.volume=target;if(!isPlaying(a)){const r=a.play();if(r&&typeof r.catch==="function")r.catch(()=>{if(fromGesture)foregroundGesturePending=true})}activeScoreMode="a";return true}catch(_){return false}}
function ensureTrackBPlayback(fromGesture=false){const b=scoreMedia("b"),target=scoreTarget();activeScoreMode="b";pauseElement(scoreMedia("a"));if(target<=0){if(b)try{b.volume=0}catch(_){};return true}if(!b)return false;try{b.muted=false;b.volume=target}catch(_){};if(isPlaying(b)){trackBPrimed=true;foregroundGesturePending=false;return true}if(!trackBPrimed&&!fromGesture){foregroundGesturePending=true;return false}let r;try{r=b.play();trackBStartCount++}catch(_){foregroundGesturePending=true;return false}if(r&&typeof r.then==="function")r.then(()=>{trackBPrimed=true;foregroundGesturePending=false}).catch(()=>{foregroundGesturePending=true});return true}
function ensureScorePlaying(fromGesture=false){if(!isP2()||document.hidden||backgroundPaused||!soundOn())return false;if(connectOpen){holdConnectSilence();return true}const desired=scoreMode(),target=scoreTarget();if(target<=0){pauseElement(scoreMedia("a"));const b=scoreMedia("b");if(b)try{b.volume=0}catch(_){};return false}const ok=desired==="b"?ensureTrackBPlayback(fromGesture):ensureTrackA(fromGesture);const room=media("ch5P2RoomTone");if(room){try{room.loop=true;room.muted=false;room.volume=roomTarget();if(room.volume>0&&room.paused){const r=room.play();if(r&&typeof r.catch==="function")r.catch(()=>{})}else if(room.volume<=0)room.pause()}catch(_){}}return ok}
function syncAudio(){if(!isP2()){pauseElement(scoreMedia("a"));pauseElement(scoreMedia("b"));pauseElement(media("ch5P2RoomTone"));return false}if(document.hidden||backgroundPaused)return false;return ensureScorePlaying(false)}
/* R19: Track B uses one WebM-first HTMLMediaElement acquired from OPEN SECURE CONTACT. */
function scheduleAudioStability(){requestAnimationFrame(()=>{if(isP2()&&!document.hidden&&!backgroundPaused)syncAudio()})}
function stopAudio(reset=false){pauseElement(scoreMedia("a"));const a=scoreMedia("a"),b=scoreMedia("b");if(a&&reset)try{a.currentTime=0}catch(_){};if(b){cancelFade(b);try{b.pause();b.volume=0;if(reset)b.currentTime=0}catch(_){}}clearTrackBPrimeTimer();trackBPrimePromise=null;trackBPrimed=false;trackBPrimeActive=false;trackBPrimeStartTime=0;pauseElement(media("ch5P2RoomTone"));if(reset)try{media("ch5P2RoomTone").currentTime=0}catch(_){};pauseElement(media("ch5P2Reveal"));if(reset)try{media("ch5P2Reveal").currentTime=0}catch(_){};stopHandshakePlayback(reset);activeScoreMode="a"}
...
function pauseForBackground(){
 if(backgroundPaused||!isP2())return false;pauseCardAuto();pauseConnectionTransition();pauseHandshakePlayback();const a=scoreMedia("a"),b=scoreMedia("b"),room=media("ch5P2RoomTone");backgroundSnapshot={aTime:finiteMediaTime(a),aWasPlaying:isPlaying(a),aVolume:Number(a?.volume)||0,bTime:finiteMediaTime(b),bWasPlaying:isPlaying(b),bVolume:Number(b?.volume)||0,bPrimed:trackBPrimed,desiredMode:scoreMode(),connectOpen:Boolean(connectOpen)};backgroundPaused=true;if(a){cancelFade(a);try{a.volume=0;a.pause()}catch(_){}}if(b){cancelFade(b);try{b.volume=0;b.pause()}catch(_){}}if(room)pauseElement(room);return true
}
function resumeMediaElement(mediaEl,time,volume,onSuccess){if(!mediaEl)return false;try{if(Number.isFinite(time))mediaEl.currentTime=time;mediaEl.muted=false;mediaEl.volume=clamp(volume,0,.72);const r=mediaEl.play();if(r&&typeof r.then==="function")r.then(()=>onSuccess?.()).catch(armForegroundGestureRecovery);else onSuccess?.();return true}catch(_){armForegroundGestureRecovery();return false}}
function resumeForeground(){
 if(document.hidden)return false;const snap=backgroundSnapshot;if(backgroundPaused){backgroundPaused=false;if(snap?.desiredMode==="b"&&snap?.bWasPlaying){trackBPrimed=Boolean(snap.bPrimed);resumeMediaElement(scoreMedia("b"),snap.bTime,scoreTarget(),()=>{activeScoreMode="b";pauseElement(scoreMedia("a"));foregroundGesturePending=false})}else if(snap?.connectOpen&&snap?.bWasPlaying){trackBPrimed=Boolean(snap.bPrimed);resumeMediaElement(scoreMedia("b"),snap.bTime,0,()=>{activeScoreMode="a";foregroundGesturePending=false})}else if(snap?.aWasPlaying){resumeMediaElement(scoreMedia("a"),snap.aTime,scoreTarget(),()=>{activeScoreMode="a";foregroundGesturePending=false})}resumeConnectionTransition();backgroundSnapshot=null;resumeCardAuto()}
...
function guardBackgroundP2Play(event){if(!backgroundPaused)return;const a=event.target;if(!a)return;try{a.pause()}catch(_){} }
function foregroundGestureRecovery(){if(document.hidden||backgroundPaused||!isP2()||!soundOn())return;if(scoreMode()==="b"){void startTrackBDirectFromGesture(false).then(ok=>{foregroundGesturePending=!ok})}else ensureTrackA(true);resumeConnectionTransition()}
...
function prepareTrackBForNarin(){prepareTrackBElement();void preloadHandshakeBuffer();return true}
...
function reassertNarinTrackBFromGesture(){const p=phaseState();if(p)p.narinMusicActive=true;if(!isP2()||document.hidden||backgroundPaused||!soundOn()||scoreMode()!=="b")return Promise.resolve(false);if(scoreTarget()<=0)return Promise.resolve(true);const b=scoreMedia("b");if(isPlaying(b)){try{b.volume=scoreTarget();b.muted=false}catch(_){};return Promise.resolve(true)}return startTrackBDirectFromGesture(false)}
...
function finishNarinReveal(){if(!connectOpen)return false;if(scoreTarget()>0&&!promoteTrackBForNarin())return setConnectAudioRetry();stopHandshakePlayback(true);connectOpen=false;connectPhase="idle";connectRemaining=CONNECT_HANDSHAKE_MS;const n=$("#ch5P2Connect");n?.classList.remove("open","established");n?.setAttribute("aria-hidden","true");return openNarinDirect()}
function retryNarinAudioFromGesture(){if(connectPhase!=="audio-retry")return false;const status=$("#ch5P2ConnectStatus");if(status)status.textContent=tr("RE-ESTABLISHING AUDIO CHANNEL","กำลังเชื่อมต่อช่องสัญญาณเสียงใหม่");void startTrackBDirectFromGesture(true).then(ok=>{if(!ok)return setConnectAudioRetry();trackBPrimed=true;finishNarinReveal()});return true}
function startNarinConnection(reset=true){
 const p=phaseState();p.narinContactStarted=true;prepareTrackBForNarin();
 if(p.narinChannelEstablished){p.stage="narin-contact";p.narinMusicActive=true;void startTrackBDirectFromGesture(false).then(ok=>{if(scoreTarget()<=0||ok)openNarinDirect();else setConnectAudioRetry()});return}
 p.narinMusicActive=false;p.stage="narin-connecting";connectOpen=true;connectPhase="handshake";connectRemaining=CONNECT_HANDSHAKE_MS;holdConnectSilence();const n=$("#ch5P2Connect");n?.classList.add("open");n?.classList.remove("established");n?.setAttribute("aria-hidden","false");updateLanguage();trackBPrimePromise=primeTrackBFromGesture(true);void playHandshake(reset);save("ch5_p2_narin_connecting");scheduleConnectTimer()
}
function revealNarinWhenScoreReady(){const ready=scoreTarget()<=0?Promise.resolve(true):(trackBPrimePromise||Promise.resolve(trackBPrimed&&trackBPlaying()));Promise.resolve(ready).then(ok=>{if(!connectOpen)return;if(ok&&promoteTrackBForNarin())finishNarinReveal();else setConnectAudioRetry()});return true}
...
function closeConnection(reset=false){connectOpen=false;connectPhase="idle";connectRemaining=CONNECT_HANDSHAKE_MS;clearConnectTimer();const n=$("#ch5P2Connect");n?.classList.remove("open","established");n?.setAttribute("aria-hidden","true");stopHandshakePlayback(reset);if(!narinOpen)stopTrackBPlayback(reset)}
...
function qaInfo(){const s=gs()||{},p=phaseState();return["LAST WITNESS QA","Build: "+String(window.LastWitnessRuntimeBuild||"0.22.22"),"Access: NORTH QA","QA Module: "+String(window.LastWitnessNorthQA?.version||"0.22.22"),"Chapter V Phase II: "+VERSION,"Chapter: 5","Phase: 2","Screen: "+activeScreen(),"Checkpoint: "+(s.checkpoint||"unresolved"),"Stage: "+(p?.stage||"unresolved"),"Language: "+(s.language==="th"?"TH":"EN"),"Visibility: "+(document.hidden?"hidden":"visible")].join("\n")}
...
window.LastWitnessChapter5Phase2={version:VERSION,installed:true,install,start,startFreshForDev,resumeFromState,stopAudio,openReconciliation:openRecon,getState:()=>clone(phaseState()),audioState:()=>({mode:scoreMode(),activeMode:activeScoreMode,aPaused:scoreMedia("a")?.paused,aTime:scoreMedia("a")?.currentTime||0,aVolume:scoreMedia("a")?.volume||0,bBackend:"html-media-user-primed",bActive:trackBPlaying(),bPaused:scoreMedia("b")?.paused,bTime:trackBPosition(),bVolume:scoreMedia("b")?.volume||0,bStarts:trackBStartCount,bPrimed:trackBPrimed,bPrimeActive:trackBPrimeActive,bCurrentSrc:scoreMedia("b")?.currentSrc||"",audioContext:secureCallCtx?.state||"none",connectOpen,connectPhase,backgroundPaused,foregroundGesturePending}),screens:[...SCREENS],qaInfo,installP1AutoHandoff};

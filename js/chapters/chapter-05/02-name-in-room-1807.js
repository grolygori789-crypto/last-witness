/* LAST WITNESS - Chapter V / Phase II: NAME IN ROOM 1807 0.22.21-c5p2r17
 * Production module update under base Runtime 0.22.21.
...
let trackBBuffer=null,trackBBufferPromise=null,trackBSource=null,trackBGain=null,trackBAnalyser=null,trackBSignalData=null,trackBStartedAt=0,trackBScheduledAt=0,trackBOffset=0,trackBUnlockPromise=null,trackBStartCount=0,connectMusicDueAt=0;
...
function preloadTrackBBuffer(forceReload=false){
 ...
 trackBBufferPromise=fetch(AUDIO+"name-changes-everything.mp3?v=0241c5p2a4",{cache:forceReload?"reload":"force-cache"}).then(r=>{if(!r.ok)throw new Error("track-b "+r.status);return r.arrayBuffer()}).then(b=>ctx.decodeAudioData(b.slice(0))).then(buf=>{if(!buf||!Number.isFinite(buf.duration)||buf.duration<=0)throw new Error("track-b decode");trackBBuffer=buf;return buf}).catch(error=>{console.error("LAST WITNESS CH5P2 Track B preload failed",error);trackBBufferPromise=null;return null});return trackBBufferPromise
}
...
function trackBPlaying(){return Boolean(trackBSource&&secureCallCtx&&secureCallCtx.state==="running"&&(!trackBScheduledAt||secureCallCtx.currentTime>=trackBScheduledAt))}
function trackBRms(){if(!trackBAnalyser)return 0;try{const n=trackBAnalyser.fftSize||2048;if(!trackBSignalData||trackBSignalData.length!==n)trackBSignalData=new Float32Array(n);trackBAnalyser.getFloatTimeDomainData(trackBSignalData);let sum=0;for(let i=0;i<trackBSignalData.length;i++){const v=trackBSignalData[i];sum+=v*v}return Math.sqrt(sum/trackBSignalData.length)}catch(_){return 0}}
...
function disconnectTrackBGraph(){if(trackBSource){try{trackBSource.disconnect()}catch(_){}}if(trackBGain){try{trackBGain.disconnect()}catch(_){}}if(trackBAnalyser){try{trackBAnalyser.disconnect()}catch(_){}}trackBGain=null;trackBAnalyser=null;trackBSignalData=null}
function stopTrackBPlayback(reset=false){const ctx=secureCallCtx,buf=trackBBuffer;if(trackBSource){if(ctx&&buf&&trackBStartedAt>0&&ctx.currentTime>=trackBStartedAt)trackBOffset=(trackBOffset+Math.max(0,ctx.currentTime-trackBStartedAt))%buf.duration;try{trackBSource.onended=null;trackBSource.stop()}catch(_){};disconnectTrackBGraph();trackBSource=null}else disconnectTrackBGraph();trackBStartedAt=0;trackBScheduledAt=0;if(reset)trackBOffset=0}
function createTrackBGraph(offset=0,when=0){
 const legacyB=media("ch5P2MusicB");if(legacyB){try{legacyB.pause();legacyB.volume=0}catch(_){}}const ctx=secureCallCtx,buf=trackBBuffer,target=scoreTarget();if(target<=0){activeScoreMode="b";pauseElement(scoreMedia());return true}if(!ctx||ctx.state!=="running"||!buf)return false;try{const src=ctx.createBufferSource(),gain=ctx.createGain(),analyser=ctx.createAnalyser();analyser.fftSize=2048;src.buffer=buf;src.loop=true;gain.gain.value=target;src.connect(gain);gain.connect(analyser);analyser.connect(ctx.destination);trackBSource=src;trackBGain=gain;trackBAnalyser=analyser;trackBOffset=Math.max(0,Math.min(offset,Math.max(0,buf.duration-.02)));const startAt=Math.max(ctx.currentTime,Number(when)||ctx.currentTime);trackBStartedAt=startAt;trackBScheduledAt=startAt>ctx.currentTime+.005?startAt:0;src.onended=()=>{if(trackBSource===src){disconnectTrackBGraph();trackBSource=null;trackBStartedAt=0;trackBScheduledAt=0}};src.start(startAt,trackBOffset);trackBStartCount++;return true}catch(error){console.error("LAST WITNESS CH5P2 Track B graph start failed",error);stopTrackBPlayback(false);return false}
}
function startTrackBPlayback(reset=true){
 const target=scoreTarget();if(target<=0){activeScoreMode="b";pauseElement(scoreMedia());return true}if(trackBSource){setTrackBGain(target);activeScoreMode="b";pauseElement(scoreMedia());return true}if(reset)trackBOffset=0;const ok=createTrackBGraph(trackBOffset,secureCallCtx?.currentTime||0);if(ok){activeScoreMode="b";pauseElement(scoreMedia());foregroundGesturePending=false}return ok
}
function scheduleTrackBForNarinReveal(){
 const ctx=secureCallCtx,target=scoreTarget();if(target<=0)return true;if(!ctx||ctx.state!=="running"||!trackBBuffer)return false;stopTrackBPlayback(true);const remaining=Math.max(.025,(Number(connectMusicDueAt)||performance.now())-performance.now())/1000;const when=ctx.currentTime+remaining;const ok=createTrackBGraph(0,when);if(!ok)return false;try{trackBGain.gain.cancelScheduledValues(ctx.currentTime);trackBGain.gain.setValueAtTime(target,when)}catch(_){};activeScoreMode="a";return true
}
function waitForScheduledTrackBSignal(timeout=1500){
 if(scoreTarget()<=0)return Promise.resolve(true);const began=performance.now();return new Promise(resolve=>{let frame=0,done=false;const finish=ok=>{if(done)return;done=true;if(frame)cancelAnimationFrame(frame);resolve(Boolean(ok))};const step=()=>{if(!connectOpen||document.hidden||backgroundPaused||!trackBSource||secureCallCtx?.state!=="running")return finish(false);const started=!trackBScheduledAt||secureCallCtx.currentTime>=trackBScheduledAt;const rms=started?trackBRms():0;if(started&&rms>0.00002)return finish(true);if(performance.now()-began>=timeout)return finish(false);frame=requestAnimationFrame(step)};frame=requestAnimationFrame(step)})
}
function holdConnectSilence(){const a=scoreMedia(),legacyB=media("ch5P2MusicB"),room=media("ch5P2RoomTone");if(a){cancelFade(a);try{a.volume=0}catch(_){}}if(legacyB){cancelFade(legacyB);try{legacyB.volume=0;legacyB.pause()}catch(_){}}if(room){cancelFade(room);try{room.volume=0;room.pause()}catch(_){}}return true}
...
function finishNarinReveal(){
 if(!connectOpen)return false;stopHandshakePlayback(true);pauseElement(scoreMedia());activeScoreMode="b";connectOpen=false;connectPhase="idle";connectRemaining=CONNECT_HANDSHAKE_MS;connectMusicDueAt=0;trackBScheduledAt=0;const n=$("#ch5P2Connect");n?.classList.remove("open","established");n?.setAttribute("aria-hidden","true");return openNarinDirect()
}
function retryNarinAudioFromGesture(){if(connectPhase!=="audio-retry")return false;const status=$("#ch5P2ConnectStatus");if(status)status.textContent=tr("RE-ESTABLISHING AUDIO CHANNEL","กำลังเชื่อมต่อช่องสัญญาณเสียงใหม่");connectMusicDueAt=performance.now()+180;void unlockP2AudioContextFromGesture(true).then(ok=>{if(!ok||!scheduleTrackBForNarinReveal())return setConnectAudioRetry();waitForScheduledTrackBSignal(1600).then(audible=>audible?finishNarinReveal():setConnectAudioRetry())});return true}
function startNarinConnection(reset=true){
 const p=phaseState();p.narinContactStarted=true;
 if(p.narinChannelEstablished){p.stage="narin-contact";p.narinMusicActive=true;void unlockP2AudioContextFromGesture(false).then(ok=>{if(scoreTarget()<=0){openNarinDirect();return}if(!ok||!startTrackBPlayback(false))return setConnectAudioRetry();requestAnimationFrame(()=>{if(trackBRms()>0.00002)openNarinDirect();else setConnectAudioRetry()})});return}
 p.narinMusicActive=false;p.stage="narin-connecting";connectOpen=true;connectPhase="handshake";connectRemaining=CONNECT_HANDSHAKE_MS;connectMusicDueAt=performance.now()+CONNECT_HANDSHAKE_MS+CONNECT_ESTABLISHED_MS;stopTrackBPlayback(true);holdConnectSilence();const n=$("#ch5P2Connect");n?.classList.add("open");n?.classList.remove("established");n?.setAttribute("aria-hidden","false");updateLanguage();prepareTrackBForNarin();const unlock=unlockP2AudioContextFromGesture(false);void unlock.then(ok=>{if(ok&&connectOpen&&connectPhase!=="audio-retry"&&!trackBSource)scheduleTrackBForNarinReveal()});void playHandshake(reset);save("ch5_p2_narin_connecting");scheduleConnectTimer()
}
function revealNarinWhenScoreReady(){const ready=scoreTarget()<=0?Promise.resolve(true):(trackBUnlockPromise||Promise.resolve(false));Promise.resolve(ready).then(ok=>{if(!connectOpen)return;if(!ok)return setConnectAudioRetry();if(scoreTarget()>0&&!trackBSource&&!scheduleTrackBForNarinReveal())return setConnectAudioRetry();return waitForScheduledTrackBSignal(1600).then(audible=>{if(!connectOpen)return;if(audible)finishNarinReveal();else setConnectAudioRetry()})});return true}
...
function closeConnection(reset=false){connectOpen=false;connectPhase="idle";connectRemaining=CONNECT_HANDSHAKE_MS;connectMusicDueAt=0;if(trackBScheduledAt)stopTrackBPlayback(reset);clearConnectTimer();const n=$("#ch5P2Connect");n?.classList.remove("open","established");n?.setAttribute("aria-hidden","true");stopHandshakePlayback(reset)}
...
function qaInfo(){const s=gs()||{},p=phaseState();return["LAST WITNESS QA","Build: "+String(window.LastWitnessRuntimeBuild||"0.22.21"),"Access: NORTH QA","QA Module: "+String(window.LastWitnessNorthQA?.version||"0.22.21"),"Chapter V Phase II: "+VERSION,"Chapter: 5","Phase: 2","Screen: "+activeScreen(),"Checkpoint: "+(s.checkpoint||"unresolved"),"Stage: "+(p?.stage||"unresolved"),"Language: "+(s.language==="th"?"TH":"EN"),"Visibility: "+(document.hidden?"hidden":"visible")].join("\n")}

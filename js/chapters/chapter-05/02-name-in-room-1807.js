/* LAST WITNESS - Chapter V / Phase II: NAME IN ROOM 1807 0.22.8-c5p2r2
...
function pauseForBackground(){if(backgroundPaused||!isP2())return;pauseCardAuto();backgroundPaused=true;ensureLoopers();backgroundSnapshot={mode:scoreMode(),room:Boolean(media("ch5P2RoomTone")&&!media("ch5P2RoomTone").paused)};loopA.pause();loopB.pause();media("ch5P2RoomTone")?.pause()}
function resumeForeground(){if(!backgroundPaused||document.hidden)return;backgroundPaused=false;const snap=backgroundSnapshot;backgroundSnapshot=null;if(!isP2())return;syncAudio(true);resumeCardAuto();if(snap?.room&&media("ch5P2RoomTone"))media("ch5P2RoomTone").play().catch(()=>{})}
...
function advanceNarin(){const p=phaseState();playOne("ch5P2Click1",.28);if(p.narinLine>=NARIN_LINES.length-1){p.narinContactComplete=true;p.stage="closing";const s=gs();s.flags.ch5_p2_narin_contact_complete=true;setProgress(94);save("ch5_p2_narin_contact_complete");closeNarin(false);startDialogue(closingLines(),()=>{const live=phaseState();live.closingComplete=true;live.stage="closing_complete";setProgress(99);save("ch5_p2_closing_complete");const b=$("#"+WORK+"Action");if(b){b.hidden=false;b.textContent=tr("COMPLETE PHASE II","จบเฟส II")}});return}p.narinLine++;renderNarin();save()}
...
function updateLanguage(){
 ...
 ch5P2RecordsLocation:tr("POLICE STATION · RESTRICTED RECORDS","สถานีตำรวจ · RESTRICTED RECORDS"),ch5P2RecordsScene:"",ch5P2RecordsObjective:tr("Resolve the protected Room 1807 identity without outrunning the record.","ยืนยันตัวตนที่ถูกปกป้องของห้อง 1807 โดยไม่สรุปไกลเกินหลักฐาน"),ch5P2RecordsNote:"NORTH · OFF RECORD",
 ch5P2WorkLocation:tr("RESTRICTED WORKSTATION · INTERNAL","RESTRICTED WORKSTATION · INTERNAL"),ch5P2WorkScene:"",ch5P2WorkObjective:tr("Bridge the protected identity path, then separate concealment from attribution.","เชื่อมเส้นทางตัวตนที่ถูกปกป้อง แล้วแยกการปกปิดออกจากการระบุตัวผู้กระทำ"),ch5P2WorkNote:"ACCESS LOGGED · READ / RECONCILE ONLY",
...
function startFreshForDev(){inject();stopForeignMedia();stopAudio(true);closeRecon(false);closeNarin(false);const s=gs();if(!s)return false;clearP2StateForDev();s.chapter=5;s.flags=s.flags||{};s.flags.ch5_p1_complete=true;s.flags.ch5_p1_room1807_identity_reference=true;s.checkpoint="ch5_p2_phase_card";s.progress=0;const p=phaseState();p.started=true;registerContent();showCard(true);save("ch5_p2_phase_card");return true}
...
function install(){inject();installSaveRestoreBridge();installQaInterceptors();installP1AutoHandoff();scheduleIntegrations();document.addEventListener("click",event=>{if(event.target.closest?.("#developerMenuButton,#northQaMenuButton,#northQaTitleButton,#settingsVersion"))setTimeout(()=>{installDevButton();installQaButton();installP1AutoHandoff()},0)},true);document.addEventListener("visibilitychange",()=>{if(document.hidden)pauseForBackground();else setTimeout(resumeForeground,30)});window.addEventListener("pagehide",pauseForBackground);window.addEventListener("pageshow",()=>setTimeout(resumeForeground,30));window.addEventListener("focus",()=>{if(!document.hidden)setTimeout(resumeForeground,30)});resumeSavedEntry();return true}

window.LastWitnessChapter5Phase2={version:VERSION,installed:true,install,start,startFreshForDev,resumeFromState,stopAudio,openReconciliation:openRecon,getState:()=>clone(phaseState()),screens:[...SCREENS],qaInfo,installP1AutoHandoff};

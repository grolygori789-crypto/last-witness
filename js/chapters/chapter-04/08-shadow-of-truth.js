...
function clearTimer(){clearTimeout(transitionTimer);transitionTimer=0;clearTimeout(takeoffRouteTimer);takeoffRouteTimer=0}
...
function dialogueDuck(){return dialogue?.56:1}
function overlayDuck(){return matrixOpen?0.48:(evidenceOpen||choiceOpen||bangkokChoiceOpen)?0.58:1}
...
function syncAudio(){
 ...
 const investigation=[OPENING,LOCATION,DEBRIEF].includes(screen),debrief=screen===DEBRIEF,travelScene=[DEPARTURE,TAKEOFF,COMPLETE].includes(screen),take=screen===TAKEOFF,departureTransition=screen===DEPARTURE;
 [investigate,travel,room,roomBase].forEach(m=>{if(m)m.loop=true});
 /* Phase VIII owns two dedicated score beds.
    Investigation C4P8 begins with the opening cinematic and stays through Secure Debrief.
    Departure C4P8 crossfades in at the airport and carries through takeoff + Chapter IV Complete.
    Source loudness differs by ~3.6 LU, so runtime gains deliberately compensate instead of flattening the masters. */
 fade(investigate,on&&investigation?music*.40*duck:0,departureTransition?1500:(screen===OPENING?850:520));
 fade(travel,on&&travelScene?music*(screen===COMPLETE?.235:take?.28:.265)*duck:0,departureTransition?1650:520);
 ...
 const ov=$("#ch4P8OpeningVideo"),tv=$("#ch4P8TakeoffVideo");if(ov)ov.volume=on?clamp(sfx*.22,0,.25):0;if(tv)tv.volume=0
}
...
function installAudioLifecycle(){
 if(audioLifecycleInstalled)return;audioLifecycleInstalled=true;
 document.addEventListener("visibilitychange",()=>{if(document.visibilityState==="hidden")pauseForBackground();else resumeForegroundAudio()});
...
function inject(){
 ...
 <div id="ch4P8RemovalCard" class="ch4-p8-removal-card" role="dialog" aria-modal="true" aria-hidden="true"><span id="ch4P8RemovalEye"></span><strong>NORTH</strong><b id="ch4P8RemovalStatus"></b><small id="ch4P8RemovalNote"></small><button id="ch4P8RemovalContinue" class="primary" type="button"></button></div>
 <audio id="ch4P8InvestigationMusic" preload="auto" loop><source src="${AUDIO}investigation-c4p8.mp3?v=0221" type="audio/mpeg"></audio><audio id="ch4P8TravelMusic" preload="auto" loop><source src="${AUDIO}departure-c4p8.mp3?v=0221" type="audio/mpeg"></audio>
...
function playOpening(){const p=ensure(),v=$("#ch4P8OpeningVideo");p.started=true;p.openingSeen=false;p.stage="opening";stopForeignAudio();stopAudio(true);safeShow(OPENING);setCheckpoint("ch4_phase8_opening");updateLanguage();if(v){try{v.currentTime=0;v.load()}catch(_){}syncAudio();v.play().then(()=>$("#ch4P8OpeningPlay")?.setAttribute("hidden","")).catch(()=>$("#ch4P8OpeningPlay")?.removeAttribute("hidden"))}}
function finishOpening(event){if(event){event.preventDefault?.();event.stopPropagation?.();event.stopImmediatePropagation?.()}const p=ensure();if(p.openingSeen)return;p.openingSeen=true;p.stage="location";stopElement($("#ch4P8OpeningVideo"),true);setCheckpoint("ch4_phase8_location");showLocation();save()}
...
function finishDebrief(){const p=ensure();p.closingDebriefComplete=true;p.stage="departure";setCheckpoint("ch4_phase8_departure");safeShow(DEPARTURE);const action=$("#"+DEPARTURE+"Action");if(action)action.hidden=true;updateLanguage();syncAudio();setTimeout(()=>showRemovalRecord(),520);save()}
function showDepartureReady(){
 const p=ensure();p.stage="departure-ready";safeShow(DEPARTURE);updateLanguage();syncProgress();syncAudio();const action=$("#"+DEPARTURE+"Action");if(action){action.hidden=false;action.textContent=tr("BOARD RETURN FLIGHT","ขึ้นเที่ยวบินขากลับ");action.onclick=()=>{action.hidden=true;playTakeoff()}};setCheckpoint("ch4_phase8_departure_ready");save()
}
function showRemovalRecord(){
 const p=ensure();p.departureSeen=true;const card=$("#ch4P8RemovalCard"),button=$("#ch4P8RemovalContinue");
 card?.classList.add("show");card?.setAttribute("aria-hidden","false");collectEvidence("ch4_p8_north_public_removal");
 if(button){button.disabled=false;button.onclick=()=>{button.disabled=true;card?.classList.remove("show");card?.setAttribute("aria-hidden","true");talk(D.departure,()=>{p.northRemovalPreserved=true;gs().flags.ch4_p8_north_publicly_removed=true;gs().flags.ch4_p8_registrar_lead_ready=true;recomputeCase();save();showDepartureReady()})}}
}
...
function returnTitle(){stopAudio(true);try{if(typeof window.LastWitnessChapter2Integration?.returnToTitle==="function")window.LastWitnessChapter2Integration.returnToTitle();else typeof show==="function"&&show("title")}catch(_){}setTimeout(()=>{try{window.LastWitnessChapter2Integration?.titleAudioState?.()}catch(_){}},0)}
...
function updateLanguage(){if(!$("#"+OPENING))return;const map={
 ...
 ch4P8DepartureLocation:tr("SOEKARNO-HATTA · INTERNATIONAL DEPARTURES","SOEKARNO-HATTA · อาคารผู้โดยสารขาออกระหว่างประเทศ"),[DEPARTURE+"Scene"]:tr("DAY 6 · 16:42 WIB","วันที่ 6 · 16:42 น."),[DEPARTURE+"Objective"]:tr("Leave Jakarta with the record intact and one lawful lead for Bangkok.","ออกจาก Jakarta โดยรักษาบันทึกเดิมไว้ และถือเบาะแสที่ชอบด้วยกฎหมายกลับกรุงเทพฯ"),[DEPARTURE+"Action"]:tr("BOARD RETURN FLIGHT","ขึ้นเที่ยวบินขากลับ"),ch4P8FlightHead:tr("SOEKARNO-HATTA · RETURN FLIGHT","SOEKARNO-HATTA · เที่ยวบินขากลับ"),ch4P8FlightStatus:tr("TAKEOFF · 18:10 WIB","เครื่องขึ้น · 18:10 WIB"),ch4P8TakeoffEye:tr("JAKARTA → BANGKOK","JAKARTA → BANGKOK"),ch4P8TakeoffTitle:tr("RETURN TO BANGKOK","เดินทางกลับกรุงเทพฯ"),ch4P8DepartLabel:tr("DEPART","ออกเดินทาง"),ch4P8DestinationLabel:tr("DESTINATION","ปลายทาง"),ch4P8TakeoffTime:tr("SOEKARNO-HATTA · TAKEOFF 18:10 WIB · CASE TRANSFER CONTINUES","SOEKARNO-HATTA · เครื่องขึ้น 18:10 WIB · การสืบสวนเดินหน้าต่อ"),ch4P8TakeoffContinue:tr("CONTINUE","ดำเนินต่อ"),ch4P8TakeoffSkip:tr("SKIP","ข้าม"),
 ...
 ch4P8RemovalEye:tr("PUBLIC PERSONNEL RECORD","บันทึกบุคลากรที่เปิดเผยภายนอก"),ch4P8RemovalStatus:tr("REMOVED","REMOVED"),ch4P8RemovalNote:tr("No correction published","ยังไม่มีการเผยแพร่การแก้ไข"),ch4P8RemovalContinue:tr("CONTINUE","ดำเนินต่อ"),

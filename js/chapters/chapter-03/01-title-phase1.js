 ...
 * Defect repair 0.4.1: silent intro, refined mix, persistent Character Journal, puzzle cleanup.
 */
(function(){
"use strict";
...
const game={
 language:localStorage.getItem("last_witness_language")||"en",route:null,approach:null,checkpoint:"entry",history:[],evidence:[],selected:[],puzzleComplete:false,dialogue:null,dialogueIndex:0,sound:true,music:.33,sfx:.55,characters:{Benedict:true,North:true,Elena:true,Somchai:true,Kittisak:true,Ratchata:true},
...
const AUDIO_MIX={officeAmbience:.36,officeScore:.13,click:.72,page:.76};
const fadeTimers=new WeakMap();
function play(id,volume){if(!game.sound)return;const a=$(id);if(!a)return;try{a.currentTime=0;a.volume=Math.max(0,Math.min(1,volume));a.play().catch(()=>{})}catch(_){}}
...
function fadeLoop(audio,target,duration=1100){if(!audio||!game.sound)return;const previous=fadeTimers.get(audio);if(previous)clearInterval(previous);audio.loop=true;if(audio.paused){audio.volume=0;audio.play().catch(()=>{})}const start=Number(audio.volume)||0;const began=performance.now();const timer=setInterval(()=>{const p=Math.min(1,(performance.now()-began)/duration);audio.volume=start+(target-start)*p;if(p>=1){clearInterval(timer);fadeTimers.delete(audio)}},40);fadeTimers.set(audio,timer)}
function click(){play("#ch3ClickAudio",game.sfx*AUDIO_MIX.click)}
...
function startOfficeAudio(){
 const ambience=$("#ch3OfficeAudio"),music=$("#ch3MusicAudio");
 if(!game.sound){stopOfficeAudio();return}
 fadeLoop(ambience,game.music*AUDIO_MIX.officeAmbience,900);
 fadeLoop(music,game.music*AUDIO_MIX.officeScore,1350);
}
...
function begin(route){click();const source=readChapter2();game.route=route;game.approach=null;game.history=[];game.evidence=[];game.selected=[];game.puzzleComplete=false;game.checkpoint="intro";if(source){game.relationships=source.relationships||game.relationships;game.personality=source.personality||game.personality;game.characters=Object.assign({},game.characters,source.characters||{})}
...
function closePuzzle(hideLauncher=false){$("#ch3PuzzleModal").classList.remove("open");if(hideLauncher)$("#ch3PuzzleButton").classList.remove("show")}
function openPuzzle(){if(game.puzzleComplete)return;click();renderPuzzle();$("#ch3PuzzleModal").classList.add("open");game.checkpoint="puzzle";save(true)}
...
function addEvidence(key,description){if(game.evidence.some(e=>e.key===key))return;game.evidence.push({key,description});play("#ch3PageAudio",game.sfx*AUDIO_MIX.page)}
